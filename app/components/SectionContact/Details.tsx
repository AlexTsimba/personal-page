...
export default function Details({ dict }: DetailsProps) {
  const details = [
    {
      title: dict.region,
      icon: <MapPin className="text-accent2-foreground" size={30} />,
      content: dict.currentLocation,
    },
    {
      title: dict.telephone,
      icon: <PhoneCall className="text-accent2-foreground" size={30} />,
      content: '+38 (068) 522-22-44',
      action: 'tel',
    },
    {
      title: dict.mailto,
      icon: <AtSign className="text-accent2-foreground" size={30} />,
...

    {
      ...
      icon: <Linkedin className="text-accent2-foreground" size={30} />,
      content: '/AlexTsimba',
    },
  ];
  ...
  return (
    <div className="flex h-full w-full flex-col gap-10">
      <div className="flex flex-col gap-14 font-medium">
        <span className="text-3xl font-bold leading-relaxed text-accent2-foreground after:content-['?']">
          {dict.title}
        </span>
        <div>
        ...
        {details.map((detail) => (
          <div key={detail.title} className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className=" text-lg font-semibold capitalize leading-tight text-accent2-foreground after:content-[':']">
                {detail.title}
              </span>
